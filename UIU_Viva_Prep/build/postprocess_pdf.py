...
def headings_from_markdown(paths: list[Path]) -> list[Heading]:
    result: list[Heading] = []
    for path in paths:
        # A line beginning '# ' inside a code fence is not a chapter heading.
...
def find_heading_pages(doc: fitz.Document, headings: list[Heading]) -> list[list[object]]:
    # TOC entries contain the same text as body headings. Match actual large
    # heading blocks only, otherwise bookmarks point back into the contents.
    page_text = []
    for page in doc:
        blocks = []
        for block in page.get_text("dict")["blocks"]:
            ...
            if not any(s["size"] >= 16.5 for s in spans):
                continue
...
def main() -> int:
    ...
    metadata.update(
        {
            "title": "Bismillah. UIU CSE Lecturer - Demo and Interview Preparation",
            "author": "Ahmmad Nur Swapnil",
            "subject": "UIU September 14, 2026: structured programming, OOP, discrete math, data structures, algorithms, DBMS, SWE, OS, AI and ML",
            "keywords": "UIU, CSE, lecturer, whiteboard demo, C, OOP, discrete, DSA, DBMS, SWE, OS, AI, ML",
